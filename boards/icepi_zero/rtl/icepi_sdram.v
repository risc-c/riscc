// IcePi x16 SDRAM interface with registered I/O and a phased device clock.
// The output stage and rising-edge input capture provide one full controller
// cycle. READ_DELAY aligns responses with that phase; the board uses READ_DELAY=1.
`timescale 1ns/1ps
...
module icepi_sdram #(
    ...
    // Keep the I/O-to-core data path unconditional.
    // Adjacent SDRAM beats form one bus word; mem_ack marks valid pairs.
    reg [15:0] previous_q;
    reg [31:0] response_q;
    ...
    genvar pin;
    generate
        for (pin = 0; pin < 16; pin = pin + 1) begin : g_dq_pin
`ifndef VERILATOR
...
`ifndef VERILATOR
    ...
    genvar bit_index;
    generate
        for (bit_index = 0; bit_index < 16; bit_index = bit_index + 1) begin : g_input
    ...
    endgenerate
    generate
        for (pin = 0; pin < 22; pin = pin + 1) begin : g_control
