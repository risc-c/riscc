...
module riscc_uart_mmio #(
    ...
    localparam integer DIV_BITS = CLK_DIV <= 1 ? 1 : $clog2(CLK_DIV);
    ...
    localparam [DIV_BITS-1:0] CLK_DIV_HALF = div_value(CLK_DIV - 1 - CLK_DIV / 2);
...
`endif
    ...
    wire tx_write, ctrl_write;
    wire [7:0] write_data;
    generate
        if (PIPELINE_WRITES != 0) begin : g_pipeline_writes
            reg tx_write_q, ctrl_write_q;
            reg [7:0] write_data_q;
            always @(posedge clk) begin
                tx_write_q <= !rst && cpu_we && data_sel;
                ctrl_write_q <= !rst && cpu_we && state_sel;
                write_data_q <= cpu_wdata[7:0];
            end
            assign tx_write = tx_write_q;
            assign ctrl_write = ctrl_write_q;
            assign write_data = write_data_q;
        end else begin : g_direct_writes
            assign tx_write = cpu_we && data_sel;
            assign ctrl_write = cpu_we && state_sel;
            assign write_data = cpu_wdata[7:0];
        end
    endgenerate

    assign irq = (irq_en[0] && rx_ready) || (irq_en[1] && tx_ready);
    assign cpu_rdata = data_sel ? {{(DATA_WIDTH-8){1'b0}}, rx_data} :
                       state_sel ?
...
`endif
            ...
            if (rx_bits == 4'd0) begin
                ...
                rx_div <= CLK_DIV_LAST;
                // Count 10 samples START; counts 9..2 collect the eight
                // data bits. Publish the complete byte at the stop bit.
...
`endif
                end else if (rx_bits != 4'd10) begin
