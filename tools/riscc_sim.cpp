...
// The top eight words are permanently reserved for I/O and test control.
...
struct Sim {
    ...
    void step() {
        ...
        if (opts.has_sys() && ie && (irq_line || uart_irq())) {
            s[0] = pc & 0x7fff;
            ie = false;
            pc = 2;
            cycles += cycle.irq_entry;
        ...
        if (pending_cycle_adjust) {
            cycles -= std::min(cycles, pending_cycle_adjust);
