#!/usr/bin/env python3
"""Run the standalone Fast pipeline cycle checks.
...
one-beat/two-beat memory contract. CASE=3 checks JALL's literal and link;
...
aliases the base or index register,
...
def one_variant(root: Path, verilator: str, build_root: Path, xlen: int,
    ...
    binary = mdir / "Vriscc_fast_pipeline_tb"
    ...
    compile_command = [
        ...
        "--top-module", "riscc_fast_pipeline_tb", f"-GXLEN={xlen}",
        "--Mdir", str(mdir), *defines,
        str(rtl_source),
        str(root / "test/riscc_fast_pipeline_tb.v"),
    ]
    ...
    if xlen == 32:
        checks.extend((8, mode, 0)
                      for mode in ("", "WAIT", "STALL", "ZERO", "MIX", "ACK_HIGH"))
        # Raise IRQ after either accepted RC32 native-load beat.  The handler
        # uses CLI/STI/RETI and checks that EPC is the following instruction.
        for beat in (1, 2):
            checks.extend((5, mode, beat)
...
def main() -> int:
    ...
    parser.add_argument("--rtl", type=Path,
                        help="Fast RTL source to compile (defaults to rtl/riscc_fast.v)")
    parser.add_argument("--verilator", default=shutil.which("verilator") or "verilator")
    parser.add_argument("--build-dir", type=Path,
                        default=Path("build/test-fast-pipeline"))
    ...
    rtl_source = args.rtl or (root / "rtl/riscc_fast.v")
    ...
    if not coverage["early"]:
        sys.stderr.write("Fast pipeline coverage failed: no same-cycle ACK\n")
        return 1
    if not coverage["ack_stalls"] or not coverage["stable"]:
        sys.stderr.write("Fast pipeline coverage failed: no ACK+STALL/stable hold\n")
        return 1
    if not coverage["ack_idle"]:
        sys.stderr.write("Fast pipeline coverage failed: no held idle ACK\n")
        return 1
    print(f"Fast pipeline cycle checks PASS ({len(variants)} variants, "
          f"38 base cases plus 16 RC32 IRQ cases; early_acks={coverage['early']} "
