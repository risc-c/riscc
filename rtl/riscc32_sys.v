// riscc32_sys.v : area-oriented serial RC32 Sys core.
//
...
`endif
    ...
    // assembled halfword stable without a separate memory-data register. JALL
    // continues to use the normal stream path.
    always @(posedge clk)
        if (stream_en)
            data_stream_q <= {
                data_stream_input,
