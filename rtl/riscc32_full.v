// riscc32_full.v : area-oriented serial RC32 Full core.
//
// This is the RC32 Full sibling of riscc32_sys. It keeps the one-port,
...
//
// Full adds IE/interrupt entry, JALL/JMPL, immediate shifts by one through
// eight, and iterative low-half multiplication.
...
`endif
    ...
    localparam [2:0] ST_DECODE = 3'b000;
    localparam [2:0] ST_FETCH_WAIT = 3'b010;
    localparam [2:0] ST_MEM_WAIT = 3'b011;
    localparam [2:0] ST_MEM_XFER = 3'b110;
    localparam [2:0] ST_INIT2 = 3'b100;
    localparam [2:0] ST_EXECUTE = 3'b111;
    localparam [2:0] ST_INIT = 3'b101;
    ...
    reg pc_phase_q;
    reg ldpc_loaded_q;
    reg [2:0] shift_remaining_q;
    reg shift_repeat_q;
    reg multiply_shift_q;
    reg [4:0] multiply_iteration_q;
    reg multiply_carry_q;
    reg ie_q;
    reg trap_q;
    // Set after the JALL header commits, while the literal is fetched and
    // then shifted through the existing data stream into the PC.
    reg jall_phase_q;
    ...
    wire full_immediate_shift_op;
    wire shift_more = data_execute & full_immediate_shift_op &
        ~trap_active & (|shift_remaining_q);
    wire pc_execute = pc_execute_raw & ~shift_more;
    wire multiply_shift;
    ...
    wire ldpc_restore_flag_op =
        deferred_ldpc_op & ~RESTORE_LDPC_IN_INIT;
    wire deferred_ldpc_pre_raw =
        (ldpc_restore_init_op & in_execute) |
        (ldpc_restore_flag_op & ~ldpc_loaded_q);
    wire deferred_ldpc_restore;
    generate
        if (W == 16) begin : g_wide_ldpc_decode
            assign deferred_ldpc_pre =
                ldpc_op & ~ldpc_loaded_q & ~trap_active;
            // The flag can only be set by this retained LDPC instruction.
            assign deferred_ldpc_restore = ldpc_loaded_q;
        end else begin : g_serial_ldpc_decode
            assign deferred_ldpc_pre = deferred_ldpc_pre_raw;
            assign deferred_ldpc_restore =
    ...
    // MUL keeps its high accumulator in address_stream_q and its multiplier
    // and low product in data_stream_q. Each bit alternates one serial add
    // pass with one serial shift pass; the final shift writes rd directly.
    wire multiply_op = register_alu_op & (f5 == 5'b00111);
    wire multiply_pass = in_mem_xfer & multiply_op;
    wire multiply_add = multiply_pass & ~multiply_shift_q;
    assign multiply_shift = multiply_pass & multiply_shift_q;
    wire multiply_write = multiply_shift & (&multiply_iteration_q);
    wire slt_op = ordinary_alu_op & ~f5[2] & f5[1];
    wire signed_compare = ~instr_q[3];
    wire right_shift_op =
        register_group & f_group_01 & f5[2] & ~f5[1];
    wire left_shift_op = register_group & (f5 == 5'b01111);
    assign full_immediate_shift_op = right_shift_op | left_shift_op;
    wire arithmetic_shift = f5[0];
    wire any_shift_op = right_shift_op | left_shift_op | funnel_right_op;
    ...
    // RET/RETI and CLI/STI share bbb=000. ddd[1] distinguishes the
    // return selectors from the direct IE controls.
    wire control_ie_value = ddd[2];
    wire control_plane = system_op & ~bbb[1];
    wire control_ie_next = (W >= 8) ?
    ...
    // Trap entry has priority at the sequencer, PC, and RF destinations, so
    // the ordinary register-target decode needs no trap mask of its own.
    wire register_target_op = system_op & ~bbb[1] &
        (bbb[0] | ~ddd[1]);
    // The unused 00 major space aliases the shared long-call path, exactly as
    // RC16 Sys does. Defined JALL/JMPL encodings retain their behavior.
    wire jall_op = ~instr_q[15] & ~instr_q[14] & ~trap_active;
    wire link_context = register_jal_op | jall_phase_q;
    wire memory_following_execute =
        ((W == 16) ? 1'b1 : ~trap_active) & deferred_ldpc_pre |
        (jall_op & ~jall_phase_q);
    ...
    wire branch_reads_r0 = branch_group & ~ddd[2];
    wire needs_operand_pass = needs_rb_pass | branch_reads_r0;
    ...
    wire hold_pc_operand = SHARE_PC_ADDER & data_execute &
        (register_target_op | (jall_phase_q & (W != 16)));
    wire stream_en = slice_count_en & ~memory_write_wait &
        ~hold_pc_operand & ~multiply_add;
    ...
    wire dst_system_bank = (system_op & bbb[0]) | jall_phase_q;
    wire source_is_rd = immediate_group | funnel_op | shift_repeat_q |
        shift_more;
    wire [2:0] rf_src_gpr = branch_reads_r0 ? 3'b000 :
        source_is_rd ? ddd : aaa;
    wire [3:0] rf_src_reg = {src_system_bank, rf_src_gpr};
    wire [3:0] rf_dst_reg = {trap_active | dst_system_bank,
        ddd & {3{~(cmpi_op | trap_active)}}};
    ...
    wire writes_data_result = immediate_alu_op |
        (register_alu_op & ~multiply_op) |
        any_shift_op | system_move_op;
    wire rf_we_normal =
        (data_execute & writes_data_result) |
        (pc_execute &
         ((link_context & link_dest_nonzero) | trap_active));
    wire rf_we = (in_mem_xfer & load_op) | multiply_write | rf_we_normal;
    wire [W-1:0] rf_rdata;
    wire [W-1:0] rf_wdata;
    ...
    wire alu_subtract =
        (ordinary_alu_op & ~multiply_op & (f5[1] | f5[0])) | cmpi_op;
    wire slt_execute = slt_op & data_execute;
    wire [W-1:0] alu_a =
        rf_rdata & {W{alu_a_enable & ~slt_execute &
                         (~multiply_add | data_stream_q[0])}};
    ...
    // ------------------------------------------------------------------
    // Branch condition scan and PC offset
    // ------------------------------------------------------------------
    reg r0_zero_q;
    reg r0_negative_q;

    always @(posedge clk)
        if (in_init2 & branch_reads_r0) begin
            r0_zero_q <= (rf_rdata == {W{1'b0}}) &
                         (first_slice | r0_zero_q);
            if (last_slice)
                r0_negative_q <= rf_rdata[W-1];
        end
    ...
    wire alu_sum_carry = multiply_add ? multiply_carry_q :
        pc_uses_alu ? pc_carry_q : alu_carry_q;
    ...
    assign alu_b_source = multiply_add ?
        address_stream_q[W-1:0] :
        needs_operand_pass ? data_stream_q[W-1:0] : imm_slice;

    always @(posedge clk)
        if (multiply_add & first_slice)
            serial_bit_q <= alu_sum[0];
        else if (in_init & first_slice)
            serial_bit_q <= data_stream_q[0];
        else if (data_execute & left_shift_op)
            serial_bit_q <= data_stream_q[W-1];
    ...
    wire [31:0] irq_pc_word = 32'h0000_0004 >> (slice_idx_q * W);
    wire [W-1:0] irq_pc_slice = irq_pc_word[W-1:0];
    wire [W-1:0] next_pc_slice;
    generate
        if (W == 16) begin : g_jall_direct_next_pc
            assign next_pc_slice = trap_active ? irq_pc_slice :
                jall_phase_q ? jall_stream_slice :
                register_target_op ? data_stream_q[W-1:0] : pc_sum;
        end else begin : g_jall_staged_next_pc
            assign next_pc_slice = trap_active ? irq_pc_slice :
                (jall_phase_q | register_target_op) ?
                    data_stream_q[W-1:0] : pc_sum;
        end
    endgenerate

    always @(posedge clk) begin
        pc_carry_q <= (in_decode & take_irq) ? 1'b0 : pc_execute ?
                      (last_slice ? 1'b0 : pc_sum_ext[W]) :
                      (SHARE_PC_ADDER ? ~trap_active : 1'b1);
        if (rst)
            pc_q <= RESET_PC;
        else if (pc_execute)
    ...
    wire [W-1:0] multiply_address_shift_slice;
    wire address_stream_en = multiply_pass |
        (in_init2 & multiply_op) | pc_execute | in_init;
    wire [W-1:0] address_stream_input = multiply_shift ?
        multiply_address_shift_slice :
        (in_init2 & multiply_op) ? {W{1'b0}} :
        pc_execute ? next_pc_slice : alu_sum;
    always @(posedge clk) begin
        if (rst)
            address_stream_q <= RESET_PC;
        else if (address_stream_en)
            address_stream_q <= {
                address_stream_input, address_stream_q[31:W]};
    end
    ...
    wire [W-1:0] left_shift_result_slice;
    wire [W-1:0] multiply_data_shift_slice;
    generate
        if (W == 1) begin : g_left_shift_w1
            assign left_shift_result_slice = first_slice ? 1'b0 :
                                             serial_bit_q;
            assign multiply_address_shift_slice = last_slice ? 1'b0 :
                address_stream_q[1];
            assign multiply_data_shift_slice = last_slice ? serial_bit_q :
                data_stream_q[1];
        end else begin : g_left_shift_wide
            assign left_shift_result_slice = {
                data_stream_q[W-2:0], first_slice ? 1'b0 : serial_bit_q};
            assign multiply_address_shift_slice = {
                last_slice ? 1'b0 : address_stream_q[W],
                address_stream_q[W-1:1]};
            assign multiply_data_shift_slice = {
                last_slice ? serial_bit_q : data_stream_q[W],
                data_stream_q[W-1:1]};
        end
    endgenerate
    ...
    // JALL's literal follows the normal serial stream direction: the low
    // half comes from memory, then the header supplies address bits 20:16 and
    // the remaining high bits are zero. This keeps the target on the usual
    // W-bit stream-input mux instead of adding a 32-bit parallel load path.
    wire [15:0] jall_upper_half = {11'b0, instr_q[10:6]};
    generate
        if (W == 16) begin : g_jall_stream_w16
            assign jall_stream_slice = upper_half ? jall_upper_half :
                                                   mem_response_q;
        end else begin : g_jall_stream_serial
            assign jall_stream_slice = upper_half ?
                jall_upper_half[stream_bit_offset[3:0] +: W] :
                mem_response_q[stream_bit_offset[3:0] +: W];
        end
    endgenerate
    generate
        if (W == 16) begin : g_jall_direct_pc
            assign data_stream_input = rf_rdata;
        end else begin : g_jall_staged_pc
            assign data_stream_input = jall_phase_q ?
                                       jall_stream_slice : rf_rdata;
        end
    endgenerate
    ...
    // Shift operands, multiplication, and stores share one serial staging
    // register. A store request is issued on the final slice of each
    // transferred halfword. If ACK is late, the slice counter and stream
    // hold, keeping RF data and the assembled halfword stable without a
    // separate memory-data register. JALL continues to use the normal stream
    // path.
    wire data_stream_en = multiply_shift | stream_en;
    wire [W-1:0] selected_data_stream_input = multiply_shift ?
        multiply_data_shift_slice : data_stream_input;
    always @(posedge clk)
        if (data_stream_en)
            data_stream_q <= {
                selected_data_stream_input,
                data_stream_q[31:W]};
    ...
    wire [W-1:0] selected_shift_slice = left_shift_op ?
        left_shift_result_slice : shift_result_slice;
    assign rf_wdata = (any_shift_op & ~trap_active) ? selected_shift_slice :
        in_mem_xfer ? (multiply_op ? multiply_data_shift_slice :
                                     direct_load_slice) :
    ...
    always @(posedge clk) begin
        ...
        case (state_q)
            ...
            ST_INIT2:
                if (last_slice)
                    state_q <= multiply_op ? ST_MEM_XFER :
            ...
            ST_MEM_XFER:
                if (multiply_op) begin
                    if (multiply_shift & last_slice &
                        (&multiply_iteration_q))
                        state_q <= ST_EXECUTE;
                end else if (~memory_write_wait) begin
                    if (last_slice)
            ...
            ST_EXECUTE:
                if (last_slice)
                    if (shift_more)
                        state_q <= ST_INIT2;
                    else if (~SHARE_PC_ADDER | pc_phase_q)
                        state_q <= memory_following_execute ? ST_MEM_WAIT :
                                                             ST_FETCH_WAIT;
            default:
        ...
        if (rst) begin
            shift_remaining_q <= 3'b000;
            shift_repeat_q <= 1'b0;
        end else if (in_decode) begin
            shift_remaining_q <= full_immediate_shift_op ? bbb : 3'b000;
            shift_repeat_q <= 1'b0;
        end else if (data_execute & last_slice &
                     full_immediate_shift_op) begin
            if (|shift_remaining_q) begin
                shift_remaining_q <= shift_remaining_q - 1'b1;
                shift_repeat_q <= 1'b1;
            end else begin
                shift_repeat_q <= 1'b0;
            end
        ...
        if (rst) begin
            multiply_shift_q <= 1'b0;
            multiply_iteration_q <= 5'b0;
            multiply_carry_q <= 1'b0;
        end else if (in_init2 & last_slice & multiply_op) begin
            multiply_shift_q <= 1'b0;
            multiply_iteration_q <= 5'b0;
            multiply_carry_q <= 1'b0;
        end else if (multiply_add) begin
            if (last_slice) begin
                multiply_shift_q <= 1'b1;
                multiply_carry_q <= 1'b0;
            end else begin
                multiply_carry_q <= alu_sum_ext[W];
            end
        end else if (multiply_shift & last_slice) begin
            multiply_shift_q <= 1'b0;
            if (~(&multiply_iteration_q))
                multiply_iteration_q <= multiply_iteration_q + 1'b1;
        end
        ...
        if (rst)
            jall_phase_q <= 1'b0;
        else if (pc_execute & last_slice)
            jall_phase_q <= jall_op & ~jall_phase_q;

        if (rst) begin
            ie_q <= 1'b0;
            trap_q <= 1'b0;
        end else begin
            if (in_decode | (pc_execute & last_slice))
                trap_q <= in_decode & take_irq;
            if (in_execute & last_slice) begin
                if (trap_active)
                    ie_q <= 1'b0;
                else if (ie_write_op)
                    ie_q <= control_ie_next;
            end
        end
    end

    generate
        if (W == 16) begin : g_wide_pc_phase
            always @(posedge clk)
                if (rst)
                    pc_phase_q <= 1'b0;
                else if (in_decode & take_irq)
                    // Interrupt entry writes only EPC and the vector; it has
                    // no data result pass at the wide datapath.
                    pc_phase_q <= 1'b1;
                else if (in_execute & last_slice & ~shift_more)
                    pc_phase_q <= deferred_ldpc_pre ? 1'b1 : ~pc_phase_q;
        end else begin : g_serial_pc_phase
            always @(posedge clk)
                if (rst)
                    pc_phase_q <= 1'b0;
                         ...
                         ~shift_more)
                    pc_phase_q <=
                        (ldpc_restore_flag_op & deferred_ldpc_pre &
                         ~trap_active) ? 1'b1 : ~pc_phase_q;
        end

        if (W == 16) begin : g_wide_ldpc_phase
            // Only /16 retains this flag. Every completed PC pass can clear
            // it; an LDPC pre-pass is the sole path that writes one.
            always @(posedge clk)
                if (rst)
                    ldpc_loaded_q <= 1'b0;
                else if (pc_execute & last_slice)
                    ldpc_loaded_q <= deferred_ldpc_pre;
        end else begin : g_serial_ldpc_phase
            always @(posedge clk)
                if (rst)
                    ldpc_loaded_q <= 1'b0;
